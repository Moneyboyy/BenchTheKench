...
namespace BenchTheKench
{
    ...
    public class SaveUnit
    {
        ...
        {
            get { return BenchTheKench.SaveMenu["save" + Unit.ChampionName].Cast<CheckBox>().CurrentValue; }
        }
    }
    ...
    class KenchSaver
    {
        ...
        public static void Initialize()
        {
            ...
            BenchTheKench.SaveMenu = BenchTheKench.Menu.AddSubMenu("Save Settings");
            BenchTheKench.SaveMenu.AddGroupLabel("Saving Allies Settings");
            BenchTheKench.SaveMenu.Add("enableSaving", new CheckBox("Saving Enabled"));
            BenchTheKench.SaveMenu.AddSeparator();

            foreach (var source in EntityManager.Heroes.Allies.Where(a => !a.IsMe))
            {
                BenchTheKench.SaveMenu.Add("save" + source.ChampionName, new CheckBox("Save " + source.ChampionName));
        ...
        {
            if (BenchTheKench.SaveMenu["enableSaving"].Cast<CheckBox>().CurrentValue && W.IsReady())
            {
        ...
        private static void OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender.IsEnemy)
            {
                foreach (var saveUnit in UnitsToSave.Where(a => a.IsEnabled))
                {
                    ...
                    if (saveUnit.Unit != null && BenchTheKench.SaveMenu["enableSaving"].Cast<CheckBox>().CurrentValue)
                    {
                        // Auto attacks
